...
<body style="margin:0px;">
    ...
    $duration[] = $row["movieDuration"];
			...
			echo "<label>Title:&nbsp;</label>$name[0]<br>";
			echo "<label>Starring:&nbsp;</label>$stars[0]<br>";
			echo "<label>Details:&nbsp;</label>$details[0]<br>";
			echo "<label>Duration:&nbsp;</label>$duration[0]<br>";
			echo "<label>Rating:&nbsp;</label>$rating[0]<br>";

				$count = 0;
				$movie2 = array();
				$date1 = array();
				$movie ='select MovieAiringTime.Date, Movie.movieName from MovieAiringTime inner join Movie on
				MovieAiringTime.movieId = Movie.movieId where Movie.movieId = '.$movie1.';';
				$movie = $db->query($movie);
				if ($movie ->num_rows >0){
				while ($row = $movie->fetch_assoc()){
				...
				echo "<table border='1'>
					<tr>
						<th>Date</th>
						<th>Time</th>";
						$movie ='select MovieAiringTime.Time,MovieAiringTime.Date from MovieAiringTime inner join Movie
						on MovieAiringTime.movieId = Movie.movieId where Movie.movieId = '.$movie1.';';
						$movie = $db->query($movie);
						if ($movie ->num_rows >0){
						while ($row = $movie->fetch_assoc()){
						echo"
