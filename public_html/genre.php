...
<body style="margin:0px;">
    ...
    align-items: center; vertical-align: middle;">
		...
		<div id="content-section" style="min-height: 230px;">
            ...
            $movie8 = "SELECT genre FROM Genre where genreID =".$genre.";";
            $movie8 = $db->query($movie8);
            if ($movie8 ->num_rows >0){
              while ($row = $movie8->fetch_assoc()){
              $moviegenre1 = $row["genre"];
              }}
              echo "<h1>Genre: ".$moviegenre1."</h1>";
            echo "<table border='1'>
            <tr>
            <th>Title</th>
			<th>Stars</th>
            <th>Rating</th>
            <th>Duration</th>";
            $movie = 'select Movie.movieName, Movie.movieRating, Movie.movieStars, Movie.movieDuration from Movie inner join Movie_genre on Movie.movieID = Movie_genre.movieID inner join Genre on Movie_genre.genreID = Genre.genreID where Genre.genreID ='.$genre.';';
            $movie = $db->query($movie);   
            if ($movie ->num_rows >0){
            while ($row = $movie->fetch_assoc()){
             echo "<tr>";
             echo"<td>".$row["movieName"]."</td>";
             echo"<td>".$row["movieStars"]."</td>";
